using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

/// <summary>
/// Summary description for HoSoUngVienInfo
/// </summary>
public class HoSoUngVienInfo
{
	public HoSoUngVienInfo()
	{
		//
		// TODO: Add constructor logic here
		//
	}
    public int MaUngVien { get; set; }
    public string HoDem { get; set; }
    public string Ten { get; set; }
    public string GioiTinh { get; set; }
    public string BiDanh {get;set;}
    public string MaTinhTrangHonNhan { get; set; }
    public string NoiSinh { get; set; }
    public string MaTinhThanh { get; set; }
    public string DiaChiLienHe { get; set; }
    public string DiDong { get; set; }
    public string DTCoDinh { get; set; }
    public string MucLuongToiThieu { get; set; }
    public string MucLuongMongMuon { get; set; }
    public string MaQuocTich { get; set; }
    public string MaDanToc { get; set; }
    public string MaTonGiao { get; set; }
    public string MaTPbanThan { get; set; }
    public string MaTPGiaDinh { get; set; }
    public string Email { get; set; }
    public string MaTrinhDoVanHoa { get; set; }
    public string MaTinhTrangSucKhoe { get; set; }
    public string NhomMau { get; set; }
    public string ChieuCao { get; set; }
    public string CanNang { get; set; }
    public string SoCMT { get; set; }
    public string NoiCapCMT { get; set; }
    public DateTime? NgayCapCMT { get; set; }
    public string SoHoChieu { get; set; }
    public DateTime? NgayCapHoChieu { get; set; }
    public DateTime? NgayHetHanHoChieu { get; set; }
    public string MaNoiCapHoChieu { get; set; }
    public string MaLoaiChinhSach { get; set; }
    public DateTime? NgayThamGiaCM { get; set; }
    public string DaThamGiaQuanDoi { get; set; }
    public DateTime? NgayVaoQuanDoi { get; set; }
    public DateTime? NgayRaQuanDoi { get; set; }
    public string MaChucVuQuanDoi { get; set; }
    public string MaCapBacQuanDoi { get; set; }
    public DateTime? NgayVaoDoan { get; set; }
    public string NoiKetNapDoan { get; set; }
    public string MaChucVuDoan { get; set; }
    public DateTime? NgayVaoDang { get; set; }
    public string NoiKetNapDang { get; set; }
    public string KinhNghiem { get; set; }
    public DateTime? NgayCoTheDiLam { get; set; }
    public string QueQuan { get; set; }
    public string MaTrinhDoNgoaiNgu { get; set; }
    public string MaTrinhDoTinHoc { get; set; }
    public string MaTrinhDo { get; set; }
    public string HinhThucTuyenDung { get; set; }
    public DateTime? NgayNopHS { get; set; }
    public string MaViTriCongViec { get; set; }
    public string MaDotTuyenDung { get; set; }
    public string Anh { get; set; }
    public string NguoiLienHeTrongTruongHopKhanCap { get; set; }
    public string MaQuanHeVoiUngVien { get; set; }
    public string SoDienThoai { get; set; }
    public string UuDiem { get; set; }
    public string NhuocDiem { get; set; }
    public string SoThich { get; set; }
    public int? MaNguonTuyenDung { get; set; }
    public int? HinhThucLamViec { get; set; }
    public bool? LaThuongBinh { get; set; }
    public bool? DaTrungTuyen { get; set; }
    public string BlackListOrStore { get; set; }
    public int? MaLyDo { get; set; }






}